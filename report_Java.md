...
Лобовое решение: можно использовать цепочку массивов( поскольку врятли есть непрерывный блок оперативной памяти на 18_758_328 стркутур), который будет хранить ссылку на следующий элемент(звено). В среднем на 
18_758_328 ссылок нужно будет на каждый 1000 блок, значит всего данных нужно занять:

1) 2 * 20 = 40 байт  10 * 2 = 20 байт (запись пользователя + телефонный намер)
2) 18_758_328 / 1000 ~ 18_758 ссылок.
3) Итого 18_758 * 4 (размер ссылки) + 60 * 18_758_328 = 1_125_574_712


Другое решение: можно использовать сжатые префиксные деревья или схожие структуры и алгоритмы для сжатия данных. Префиксные деревья более предпочтительны посколько у многих номеров телефонов совпадает код страны и код оператора. а также совпадают начальные цифры, так что можно хранить как минимум в два раза меньше информации, и использовать только метки. 
